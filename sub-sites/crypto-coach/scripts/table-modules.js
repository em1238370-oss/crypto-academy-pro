...
window.runDcaExperiment = function runDcaExperiment() {
    ...
    html += '<div class="dca-expert-block dca-result-section"><h6>From a professional perspective</h6>';
...
window.runStrategyReportCard = async function runStrategyReportCard() {
    ...
    resultDiv.style.display = 'block'; resultDiv.innerHTML = '<span style="color: #ffa500;">Preparing your assessment...</span>';
    try {
        const varietySeed = Date.now() + '-' + Math.random().toString(36).slice(2, 10);
        const response = await fetch(API_URL, {
            method: 'POST',
        ...
        } else { resultDiv.innerHTML = '<span style="color: #ff6666;">Assessment unavailable. Please try again later.</span>'; }
    } catch (err) { resultDiv.innerHTML = '<span style="color: #ff6666;">Error: ' + (err.message || 'Try again') + '</span>'; }
};
