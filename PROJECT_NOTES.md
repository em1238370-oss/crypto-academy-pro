...
- 2026-05-05 — Главная `analyze-channel`: если Telethon и t.me/s не дали текста, вместо пустого `UNAVAILABLE` отдаётся полный ответ с `read_path: dataset_evidence` — фрагменты из scam_base/reports (честные цитаты, без выдумывания постов Telegram), Risk score и trust_report заполняются; в UI подпись источника «фрагменты из базы и жалоб».
