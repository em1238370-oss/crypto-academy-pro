...
**Колонки A:H (обязательно), I (опционально):**
...
|-------------|---------------|-----------------|------------|--------------|---------------|---------------|------------|----------------------------|
| @TONPumpKing | 87 | 14 | 78% | 7000₽ | 23 | 2.1млн₽ | scam | 72 |
| @CryptoElite | 45 | 7 | 32% | 4900₽ | 4 | 847к₽ | grey | |
| @RealTraderPro | 12 | 1 | 8% | нет | 0 | 0₽ | safe | |

- **I (complaint_ignore_hours)** — опционально: сколько часов администрация канала не реагирует на жалобы (целое число). Используется в формуле риска и в блоке «Жалобы» на карточке. Если используете колонку I, задайте диапазон **`scam_base!A2:I`**.
