...
def _write_confirmed_to_scam_base(confirmed_objects, client, sheet_id):
    ...
    for obj in list(confirmed_objects or []):
        ...
        if w_src < _kro_source_policy.MIN_SOURCE_WEIGHT_FOR_SCAM_BASE - 1e-6:
            ...
            print(
                'scam_base write skip: %s — суммарный вес источников %.2f < %.1f (breakdown=%s)'
                % (
                    obj.get('username') or key,
                    w_src,
                    _kro_source_policy.MIN_SOURCE_WEIGHT_FOR_SCAM_BASE,
        ...
        if (
            w_src < _kro_source_policy.MIN_SOURCE_WEIGHT_FOR_DIRECT_STATUS - 1e-6
            and strong_external
            and signal_a_crypto
        ):
            ...
            print(
                ...
                % (
                    obj.get('username') or key,
                    w_src,
                    _kro_source_policy.MIN_SOURCE_WEIGHT_FOR_OBSERVATION,
                    _kro_source_policy.MIN_SOURCE_WEIGHT_FOR_DIRECT_STATUS,
                    json.dumps(w_breakdown, ensure_ascii=False, sort_keys=True),
                    new_st,
                ),
                file=sys.stderr,
        ...
        elif w_src < _kro_source_policy.MIN_SOURCE_WEIGHT_FOR_DIRECT_STATUS - 1e-6:
            _kro_cycle_analytics_inc('write_reject_source_weight')
            print(
                'scam_base write skip: %s — пограничный вес %.2f без достаточного внешнего подтверждения/crypto signal '
                '(breakdown=%s)'
                % (
                    obj.get('username') or key,
                    w_src,
            ...
            if not _recent_posts_have_crypto_terms(obj):
                if strong_external and signal_a_crypto:
                    ...
                    print(
                        ...
                        'но есть strong_external + signal_a_crypto; записываем как %s'
                        % ((obj.get('username') or key), new_st),
                        file=sys.stderr,
                    )
                    eff_status = new_st
                    obj['status'] = new_st
