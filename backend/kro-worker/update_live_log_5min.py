...
def _normalize_log_message(msg, time_str=None):
    ...
    text = re.sub(r'\s+Источники:\s.*$', '', text)
    text = re.sub(r'\s+Группы:\s.*$', '', text)
    text = re.sub(r'\s+Ссылки:\s.*$', '', text)
    if text.startswith('Старт цикла '):
        return 'Начала новый проход по дню: открыла рабочий цикл, сверила время и перешла к поиску свежих каналов и жалоб.'
    if text.startswith('Начат сбор данных за период '):
        return 'Начала сбор за новый дневной период: сначала проверяю каталоги, затем поисковые запросы, описания каналов, закрепы и жалобы.'
...
def _phrase(kind, time_str):
    ...
    variants = {
        'intermediate': [
            'Проверила свежую выдачу по запросам о крипто‑сигналах, VIP и доходе, затем открыла описания найденных каналов. За эти 5 минут новых подтверждённых объектов не добавилось.',
            'Снова прошлась по ключевым запросам, связанным с сигналами, приватами и риск‑обещаниями, и перепроверила найденные каналы по описанию и закрепам. Новых подтверждений за слот нет.',
            'За этот интервал просмотрела каталоги и свежие результаты поиска, затем сверила формулировки про сигналы, VIP и быстрый заработок. Ничего нового, что можно подтвердить, пока не появилось.',
        ],
        'repeat_pass': [
            'Сделала повторную проверку уже найденных кандидатов: ещё раз открыла описания, закрепы и последние посты. За этот интервал новых подтверждений не появилось.',
            'Вернулась к каналам, которые уже попадали в выдачу, и перепроверила их по содержанию и обещаниям дохода. Свежих подтверждённых объектов за этот шаг не добавилось.',
            'Повторно просмотрела результаты поиска и кандидатов из предыдущих слотов, чтобы не пропустить новые признаки риска. Новых подтверждений пока нет.',
        ],
        'refresh': [
            'Обновила рабочую выдачу по основным запросам и ещё раз сверила каталоги с тем, что уже было найдено раньше. Заметных изменений за этот слот нет.',
            'Перезапустила проверку по ключевым запросам о сигналах и VIP, чтобы увидеть новые публикации или появившиеся каналы. За этот интервал ничего нового не подтвердилось.',
            'Сделала дополнительную сверку результатов поиска и каталогов по риск‑формулировкам и обещаниям дохода. За эти 5 минут новых изменений не увидела.',
        ],
        'basic': [
            'На этом шаге просмотрела каталоги и основные поисковые запросы по крипто‑сигналам, чтобы собрать первичный список кандидатов для проверки.',
            'Сейчас идёт стартовый проход: открываю каталоги и поисковую выдачу по сигналам, приватам и заработку, чтобы отобрать каналы для следующей проверки.',
            'Сделала базовый просмотр каталогов и ключевых запросов по теме крипто‑сигналов. Это первый этап перед более глубокой проверкой самих каналов.',
        ],
        'found_candidates': [
            'Нашла каналы-кандидаты через каталоги и поисковые запросы. Следом открываю их описания, закрепы и последние посты, чтобы понять, подходят ли они под критерии.',
            'В выдаче появились новые кандидаты по теме сигналов и VIP. Следующий шаг - вручную проверить описания каналов, закреплённые сообщения и свежие публикации.',
            'По текущим запросам появились каналы, которые стоит проверить глубже. Перехожу к ручному просмотру описания, закрепов и последних постов.',
        ],
        'wide': [
            'За эти 5 минут посмотрела не только каталоги, но и Telegram‑поиск, жалобные обсуждения и открытые веб‑упоминания. Новых подтверждённых каналов по нужным признакам не нашла.',
            'Сделала более широкий проход: проверила Telegram‑поиск, жалобы и внешние упоминания, чтобы понять, всплыли ли новые каналы. За этот интервал новых подтверждений нет.',
            'На этом шаге расширила поиск за пределы каталогов и просмотрела Telegram, жалобы и веб‑источники. Ничего нового, что можно уверенно подтвердить, не появилось.',
        ],
        'losses': [
...
def _build_day_lines(date_str, slot_msg, today_ddmm):
    ...
    if not day_slots:
        return []
    end_slot = day_slots[-1]
    try:
        end_h, end_m = [int(x) for x in end_slot.split(':')]
    except (ValueError, TypeError):
        return [LOG_LINE_PREFIX + '%s — %s' % (slot, slot_msg[(date_str, slot)]) for slot in day_slots]
    slots = _all_slots_up_to(end_h, end_m)
    return [LOG_LINE_PREFIX + '%s — %s' % (slot, slot_msg.get((date_str, slot), _phrase('intermediate', slot))) for slot in slots]
