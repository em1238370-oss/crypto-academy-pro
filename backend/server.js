...
// ONE-SHOT CLEANUP — удалить после выполнения
app.post('/api/kro/cleanup-exclusions', express.json(), async (req, res) => {
  const CLEANUP_TOKEN = process.env.CLEANUP_TOKEN || 'kro-cleanup-2026';
  if ((req.body?.token ?? '') !== CLEANUP_TOKEN) {
    return res.status(403).json({ error: 'forbidden' });
  }
  try {
    const client = await getKroSheetsClient();
    if (!client) return res.status(503).json({ error: 'sheets_not_configured' });

    const spreadsheetId = kroSheetId;

    // ── Helper: get numeric sheetId by name ──────────────────────────────
    ...
    // ── Delete matching rows from a sheet ────────────────────────────────
    // colIdx: 0-based column index that holds the channel handle
    async function deleteMatchingRows(sheetName, colIdx, channelsToRemove) {
      const sheetId = await getSheetIdByName(sheetName);
      if (sheetId == null) return { error: `sheet "${sheetName}" not found` };
      ...
      // Collect 1-based row indices to delete (skip header row 0 = row 1)
      const toDelete = [];
      for (let i = 1; i < rows.length; i++) {
        const cell = (rows[i][colIdx] ?? '').toString().trim().toLowerCase().replace(/^@/, '');
        if (channelsToRemove.has(cell)) toDelete.push(i); // 0-based in array = 1-based in sheet rows, but row 0 is header (sheet row 1), so sheet row = i+1
      }

      if (toDelete.length === 0) return { deleted: 0 };

      // Must delete from bottom to top to keep indices stable
      const requests = toDelete.reverse().map(i => ({
        deleteDimension: {
          range: {
            sheetId,
            dimension: 'ROWS',
            startIndex: i,   // 0-based: i=1 means second row (first data row)
            endIndex: i + 1,
          },
        },
      }));
    ...
    // ── Scam_base: delete @feel340 and @gmail (col 0 = username) ─────────
    const scamBaseResult = await deleteMatchingRows(
      'scam_base', 0, new Set(['feel340', 'gmail'])
    );

    // ── Reports: delete @TEST_AUTOMATED_CHECK (col 1 = channel) ──────────
    // First check default sheet name
    const meta = await client.sheets.spreadsheets.get({ spreadsheetId });
    const firstSheetName = meta.data.sheets[0].properties.title;
    const reportsResult = await deleteMatchingRows(
      firstSheetName, 1, new Set(['test_automated_check'])
    );

    return res.json({ ok: true, scam_base: scamBaseResult, reports: reportsResult });
  } catch (e) {
    console.error('[KRO cleanup] error:', e);
