# 🌐 Ссылка на ваш сайт

## ✅ Публичный URL сайта:

### 🚀 Основной сайт:
**https://crypto-academy-pro.onrender.com**

---

## 📋 Все страницы сайта:

1. **Главная страница:**
   - https://crypto-academy-pro.onrender.com/

2. **News:**
   - https://crypto-academy-pro.onrender.com/sub-sites/news/index.html

3. **Crypto Basics:**
   - https://crypto-academy-pro.onrender.com/sub-sites/crypto-basics/index.html

4. **Crypto Coach:**
   - https://crypto-academy-pro.onrender.com/sub-sites/crypto-coach/index.html

5. **Risk Distribution:**
   - https://crypto-academy-pro.onrender.com/sub-sites/risk-distribution/index.html

---

## 🔗 GitHub репозиторий:

**https://github.com/em1238370-oss/crypto-academy-pro**

---

## ⚙️ Деплой:

- **Провайдер:** Render
- **Сервис:** Web Service
- **URL:** https://crypto-academy-pro.onrender.com
- **Автоматический деплой:** При каждом push в main ветку

---

## 📝 Примечания:

⚠️ Если сайт не загружается:
- Render может "засыпать" при неактивности (бесплатный план)
- Первая загрузка может занять 30-60 секунд
- Проверьте статус на https://dashboard.render.com

---

## ✅ Проверка:

Откройте в браузере:
**https://crypto-academy-pro.onrender.com**

Должна открыться главная страница с чат-ботом и всеми разделами!

